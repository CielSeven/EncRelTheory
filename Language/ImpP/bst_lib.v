...
From AUXLib Require Import Axioms Feq Idents List_lemma int_auto VMap BinaryTree OrdersDecFact.
...
Definition insert_f {V : Type} (f : key -> V -> (tree V) ->  (tree V))  (x : key) (v : V) (t : tree V) :=
  match t with
  | E => T E x v E
  | T l y v' r =>
	  if x <? y
      then T (f x v l) y v' r
      else if x >? y then T l y v' (f x v r) else T l x v r
  end.


Definition insert := 
  fix insert {V : Type} (x : key) (v : V) (t : tree V) {struct t} : tree V :=
  insert_f insert x v t.
...
Fixpoint store_tree (p: addr) (tr: tree) :=
  match tr with
  | empty =>
      !! (p = 0) && emp
  | make_tree tr_l k v tr_r =>
      !! ( Int64.min_signed <= k <= Int64.max_signed ) &&
      EX pl pr: addr,
        vPV p @ vint ↦ₗ ( k) $ pm **
        vPV (p + 1) @ vint ↦ₗ ( v) $ pm **
        vPV (p + 2) @ vptr ↦ₗ pl $ pm **
        vPV (p + 3) @ vptr ↦ₗ pr $ pm **
        store_tree pl tr_l **
        store_tree pr tr_r
  end.

Fixpoint store_ptb (p2 p2_root: addr) (pt: partial_tree) :=
  match pt with
  | nil =>
      !! ( p2 = p2_root ) && emp
  | LH k v tr :: pt' =>
      EX p_fa p2_fa p_bro ,
        !! ( Int64.min_signed <= k <= Int64.max_signed ) &&
        !! ( p_fa + 2 = p2) && 
        vPV p2_fa @ vptr ↦ₗ p_fa $ pm ** 
        vPV p_fa @ vint ↦ₗ ( k) $ pm **
        vPV (p_fa + 1) @ vint ↦ₗ ( v) $ pm **
        vPV (p_fa + 3) @ vptr ↦ₗ p_bro $ pm **
        store_tree p_bro tr **
  ...
  | RH k v tr :: pt' =>
      EX p_fa p2_fa p_bro ,
        ...
        vPV p_fa @ vint ↦ₗ ( k) $ pm **
        vPV (p_fa + 1) @ vint ↦ₗ ( v) $ pm **
        vPV (p_fa + 2) @ vptr ↦ₗ p_bro $ pm **
        store_tree p_bro tr **
        store_ptb p2_fa p2_root pt'
  end.

Fixpoint store_pt (p p_root: Z) (pt: partial_tree) :=
  match pt with
  | nil =>
      !! ( p = p_root ) && emp
  | LH k v tr :: pt' =>
...
Definition store_map (p: addr) (m: mapping) :=
  EX tr: tree,
...
Theorem store_ptb_LH:
  forall p2_fa p_fa p_bro k v tr,
    ...
    isvalidptr p2_fa ->
    isvalidptr p_fa ->
    PV p2_fa @ vptr ↦ₗ p_fa $ pm **
    PV p_fa @ vint ↦ₗ k $ pm **
    PV (p_fa + 1) @ vint ↦ₗ v $ pm **
    PV (p_fa + 3) @ vptr ↦ₗ p_bro $ pm **
    store_tree p_bro tr |--
    store_ptb (p_fa + 2) p2_fa
      (LH k v tr :: nil).
Proof.
  intros.
  simpl.
...
Theorem store_ptb_RH:
  forall p2_fa p_fa p_bro k v tr,
    ...
    PV p_fa @ vint ↦ₗ k $ pm **
    PV (p_fa + 1) @ vint ↦ₗ v $ pm **
    PV (p_fa + 2) @ vptr ↦ₗ p_bro $ pm **
    store_tree p_bro tr |--
    store_ptb (p_fa + 3) p2_fa
      (RH k v tr :: nil).
Proof.
  intros.
  simpl.
...
Proof.
  ...
  + intros. 
    ...
    - simpl. 
      Intros p_fa p_bro.
      sep_apply (store_tree_make_tree p_fa);  try tauto.
...
Lemma subst_local_store_tree: forall t p x n,
  subst_local x n (store_tree pm p t) --||-- store_tree pm p t.
Proof. 
  induction t;intros.
  - simpl. rewrite subst_local_coqprop. 
    rewrite subst_local_emp.
    reflexivity.
  - simpl.
    rewrite subst_local_coqprop.
    rewrite subst_local_exp.
    apply logic_equiv_andp_mono;[reflexivity | ].
...
Lemma subst_local_store_ptb: forall P pt pt_root x n,
  subst_local x n (store_ptb pm pt pt_root P) --||-- store_ptb pm pt pt_root P.
Proof. 
  induction P;intros.
  - simpl. rewrite subst_local_coqprop. 
    rewrite subst_local_emp.
  ...
  - simpl.
    ...
    + rewrite subst_local_exp.
      ...
      rewrite IHP.
      rewrite subst_local_store_tree.
      reflexivity.
    + rewrite subst_local_exp.
      eapply ex_logic_euiqv_elim_both.
      intro p_fa.
...
Lemma subst_local_store_pt: forall P pt pt_root x n,
  subst_local x n (store_pt pm pt pt_root P) --||-- store_pt pm pt pt_root P.
Proof. 
  induction P;intros.
  - simpl. rewrite subst_local_coqprop. 
    rewrite subst_local_emp.
...
Lemma subst_global_store_tree: forall t p x n,
  subst_global x n (store_tree pm p t) --||-- store_tree pm p t.
Proof. 
  induction t;intros.
  - simpl. rewrite subst_global_coqprop. 
...
Lemma subst_global_store_ptb: forall P pt pt_root x n,
  subst_global x n (store_ptb pm pt pt_root P) --||-- store_ptb pm pt pt_root P.
Proof. 
  induction P;intros.
  - simpl. rewrite subst_global_coqprop. 
    rewrite subst_global_emp.
  ...
  - simpl.
    ...
    + rewrite subst_global_exp.
      ...
      rewrite IHP.
      rewrite subst_global_store_tree.
      reflexivity.
    + rewrite subst_global_exp.
      eapply ex_logic_euiqv_elim_both.
      intro p_fa.
...
Lemma subst_global_store_pt: forall P pt pt_root x n,
  subst_global x n (store_pt pm pt pt_root P) --||-- store_pt pm pt pt_root P.
Proof. 
  induction P;intros.
  - simpl. rewrite subst_global_coqprop. 
    rewrite subst_global_emp.
...
Ltac bst_asrt_simpl := 
    ...
    | |- context [subst_local ?x ?xv (store_tree ?pm ?p ?t) ] => erewrite (subst_local_store_tree pm t p x xv)
    | |- context [subst_local ?x ?xv (store_ptb ?pm ?p ?p2 ?l) ] => erewrite (subst_local_store_ptb pm l p p2 x xv)
    | |- context [subst_local ?x ?xv (store_pt ?pm ?p ?p2 ?l) ] => erewrite (subst_local_store_pt pm l p p2 x xv)
    | |- context [subst_global ?x ?xv (store_tree ?pm ?p ?t) ] => erewrite (subst_global_store_tree pm t p x xv)
    | |- context [subst_global ?x ?xv (store_ptb ?pm ?p ?p2 ?l) ] => erewrite (subst_global_store_ptb pm l p p2 x xv)
    | |- context [subst_global ?x ?xv (store_pt ?pm ?p ?p2 ?l) ] => erewrite (subst_global_store_pt pm l p p2 x xv)
    end).