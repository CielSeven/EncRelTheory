...
From EncRelSeq Require Import contexthoare  contexthoare_imp.
...
(**********************************          1. monad  no error              *********************************)
(*************************************************************************************************************)
Module RelImpAbsMonad.

Import practicaldeno.
Import RelHoareNormalDenoAsbMonad2.
...
Section contextual_relationaltriples.
  ...
  Definition relfuncspecs_sat (χnrm : (func -> global_cstate -> global_cstate -> Prop))
    ...
    forall (w : fspec.(rFS_With)) lst1 lm1 lmf hst1 hstmt,
      ...
      forall lgst2, 
        (χnrm f) (mk_gstate lst1.(genv) lst1.(st_mem)) lgst2 -> 
        exists hst2 hstmt2, 
  ...
  Definition valid_reltriple {A: Type} (χnrm : (func -> global_cstate -> global_cstate -> Prop)) :=
    ...
    forall lst2, (evalnrm (χnrm) c) lst1 lst2 ->
    exists hst2 hstmt2, 
  ...
  Definition valid_contextualreltriple {A: Type} (Delta : funcspecs) (Gamma : relfuncspecs)  :=
    fun  (P : relassertion) (c : Langstmts) (Q : relassertion) =>
    forall χ,
    funcspecs_sat_nrm χ Delta ->  relfuncspecs_sat χ Gamma ->
    @valid_reltriple A χ P c Q.

  Lemma relfuncspecs_equivforall : forall Γ χnrm ,
    relfuncspecs_sat χnrm Γ <-> Forall (fun '(f, fspec) => forall (w : fspec.(rFS_With)) lst1 lm1 lmf hst1 hstmt,
    join lm1 lmf lst1.(st_mem) ->
    fspec.(rFS_Pre) w ((mk_lstate lst1.(lenv) lst1.(genv) lm1), hst1, hstmt) ->
    forall lgst2, 
      (χnrm f)(mk_gstate lst1.(genv) lst1.(st_mem)) lgst2 ->
...
Section encode_contextual_triples.
  ...
  Definition valid_contextualrelT {A: Type} (Delta : funcspecs) (Gamma : relfuncspecs) 
    (P: @relasrt local_cstate Σₕ (program Σₕ A)) (cₗ: Langstmts) (Q : @relasrt local_cstate Σₕ (program Σₕ A)) := 
      forall X, valid_contextualtriple_nrm   (Delta ++ (encode_relfuncspecs Gamma))
                        (encode_asrt P X)
                        cₗ 
                        (encode_asrt Q X).
...
Section contextual_validity_soundness.
  ...
  Lemma funcspecs_equiv_relfuncspecs:  forall (Γ : relfuncspecs) (χnrm: (func -> global_cstate -> global_cstate -> Prop)),
    funcspecs_sat_nrm χnrm (@encode_relfuncspecs lgst Σₕ Γ) <-> 
    relfuncspecs_sat χnrm Γ.
  Proof.
    induction Γ;intros.
    - split;intros. 
      apply relfuncspecs_equivforall. constructor.
      cbn. apply funcspecs_nrm_equivforall. constructor.
    - split;intros.
      ...
      + clear H3.
        ...
        cbn in *.
        remember (fun r hst => (hstmt) hst1 r hst) as X.
      ...
      + apply relfuncspecs_equivforall.
        apply IHΓ.
        apply funcspecs_nrm_equivforall;auto.  }
      ...
      + clear H3.
        ...
        cbn in *.
        destruct w as [X w].
        destruct H0 as (hst1 & hstmt & HHnrm & HP).
        specialize (H2 w _ _ _  hst1 hstmt H HP) as  Hnrm.
          intros * HLeval.
          specialize (Hnrm _ HLeval).
          destruct Hnrm as (hst2 & hstmt2 & Hheval & (lm2 & ? & ?)).
          eexists.
          split;eauto.
          exists hst2, hstmt2.
          split;auto.
  ...
  Proof.
    intros;split.
    - unfold valid_contextualreltriple, valid_contextualrelT, valid_contextualtriple_nrm.
      intros HT X χ HDelta lst1 lm1 lmf Hframe HP. 
      apply funcspecs_sat_nrm_app in HDelta as [HDelta HGamma].
      ...
      specialize (HT _ _ _ _ _ Hframe HP) as  Hnrm.
      intros lst2 Heval.
      unfold encode_asrt.
      specialize (Hnrm _ Heval) as (hst2 & hstmt2 & Hheval & lm2 & Hframe2 & HQ).
      eexists.
      split;eauto.
    ...
    - unfold valid_contextualreltriple, valid_contextualrelT, valid_contextualtriple_nrm.
      ...
      specialize (HT (fun r hst => (hstmt1) hst1 r  hst) χ).
      assert (funcspecs_sat_nrm χ (Δ ++ encode_relfuncspecs Γ)).
      { apply funcspecs_sat_nrm_app.
        split;auto.
      ...
      specialize (HT H lst1 _ _ Hframe).
        intros lst2 Heval.
        assert (encode_asrt P (fun (r : A) (hst : Σₕ) => hstmt1 hst1 r hst)
        ...
        specialize (HT H0) as ?.
        specialize (H1 _ Heval) as (lm2 & Hframe2 & H2). 
        unfold encode_asrt in H2.
        destruct H2 as [hst2 [hstmt2 [? ?]]];subst.
        do 2 eexists.
        split;eauto.
  Qed. 
...
Proof.
  ...
  simpl in *.
  my_destruct local_cstate Σₕ (program Σₕ A) H0.
  my_destruct local_cstate Σₕ (program Σₕ A) H1.
  subst.
...
Proof.
  ...
  specialize (HRT  (fun r (x: Σₕ) =>  sₕ σₕ r x) _ Hcallees lst1 _ _ Hjoin).
  assert (([|lift P sₕ|](fun (r : A) (x : Σₕ) =>  sₕ σₕ r x))
  ...
  specialize (H0 x0 σₕ' (ltac:(unfold_monad;auto))).
  specialize (HHT _ x0 σₕ' HPH H0) as HHQ.
  eexists;split;eauto.
  exists x0.
  unfold link.
  eexists.
...
Definition liftL {Σₗ Σₕ: Type}  (P: @asrt Σₗ) : @binasrt Σₗ Σₕ :=
    fun '(σₗ, σₕ) =>  P σₗ.
Definition liftH {Σₗ Σₕ: Type}  (P: @asrt Σₕ) : @binasrt Σₗ Σₕ :=
    fun '(σₗ, σₕ) =>  P σₕ.
...
Proof.
  ...
  (* specialize (HHT σₕ HPH) as HHnrm. *)
  (*将high level 终止状态指定为满足条件的初始状态经过从cₕ能到达的集合*)
  specialize (HRT  (fun r (x: Σₕ) =>  (sₕ u) σₕ r x) _ Hcallees lst1 _ _ Hjoin).
  assert ((EX u0 : U, !! safeExec (Ps u0) (sₕ u0) (fun (r : A) (x : Σₕ) => (sₕ u) σₕ r x) && !! B1 u0 && P u0)
  {| lenv := lenv lst1; genv := genv lst1; st_mem := m1 |}).
  { exists u.
    unfold andp, coq_prop, liftH, liftL in *. 
    splits;try tauto.
    unfold safeExec.
    exists σₕ.
    split;try tauto.
  ...
  specialize (HHT _ r σₕ' HPH H3) as HHQ.
  eexists;split;eauto.
  exists r, v.
  unfold link, liftH, liftL, andp, coq_prop.
  simpl.
  splits;auto.
  eexists.
  eauto.
...
(* 
Lemma rh_hoare_vc_safeexec {W U V: Type}:forall Δ c (sₕ :W -> U -> program Σₕ A) P Ps Q Qs B1 B2,
  (forall X (w: W),  Δ ⊢ {{EX (u: U), !! safeExec (Ps w u) (sₕ w u) X && !! (B1 w u) && (P w u)}} c 
      {{EX (r: A) (v: V), !! safeExec (Qs w v r) (return r) X && !! (B2 w v r) && (Q w v r) }}) ->
  forall Pₕ Qₕ w,
  (forall u, Hoare (Pₕ w) (sₕ w u) (Qₕ w)) ->
  (Δ  ⊢ {{EX (u: U), link (liftH (Ps w u) && !! (B1 w u) && liftL (P w u)) (Pₕ w)}} c 
                  {{EX r v, link (liftH (Qs w v r) && !! (B2 w v r) && liftL (Q w v r)) (Qₕ w r)}}).
Proof.
  unfold valid_triple; intros * HRT * HHT.
  intros χ Hcallees lst1 * Hjoin HP.
  destruct HP as [u [σₕ [? HPH]]].
  (* specialize (HHT σₕ HPH) as HHnrm. *)
  (*将high level 终止状态指定为满足条件的初始状态经过从cₕ能到达的集合*)
  specialize (HRT  (fun r (x: Σₕ) =>  (sₕ w u) σₕ r x) w _ Hcallees lst1 _ _ Hjoin).
  assert ((EX u0 : U, !! safeExec (Ps w u0) (sₕ w u0) (fun (r : A) (x : Σₕ) => (sₕ w u) σₕ r x) && !! B1 w u0 && P w u0)
  {| lenv := lenv lst1; genv := genv lst1; st_mem := m1 |}).
  { exists u.
    unfold andp, coq_prop, liftH, liftL in *. 
    splits;try tauto.
  ...
  unfold safe, ret in H3. cbn in H3.
  specialize (H3 r σₕ' (ltac:(unfold_monad;auto))).
  specialize (HHT u _ r σₕ' HPH H3) as HHQ.
  eexists;split;eauto.
  exists r, v.
  unfold link, liftH, liftL, andp, coq_prop.
  simpl.
