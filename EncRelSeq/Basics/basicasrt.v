...
From EncRelSeq Require Import SLInterface1.
Module base_rules (L: Lang).
  Export L.
  Module asrts.

  Definition expr := @expr state.
  (* derived types *)
  ...
  Module baseprimitiverule  <: PrimitiveRuleSig asrts.
  Include DerivedNames asrts.
    ...
    Proof. unfold derivable1, iffp, impp, andp ;cbv. intros. split;auto. Qed.
    Lemma derivable1_iffp_elim1 : forall x y : expr, derivable1 (iffp x y) (impp x y).
    Proof. unfold derivable1, iffp, impp, andp ;cbv. intros. destruct H.  auto. Qed.
    ...
    Lemma logic_equiv_andp_comm : forall x y : expr, logic_equiv (andp x y) (andp y x).
    Proof. apply logic_equiv_andp_comm. Qed.
    Lemma logic_equiv_andp_assoc : forall x y z : expr, logic_equiv (andp (andp x y) z) (andp x (andp y z)).
    ...
    Lemma logic_equiv_orp_assoc : forall x y z : expr, logic_equiv (orp (orp x y) z) (orp x (orp y z)).
    Proof. apply logic_equiv_orp_assoc. Qed.

  End baseprimitiverule.
  Module baserules := LogicTheorem asrts baseprimitiverule.
  Arguments baserules.exp {A}.
End base_rules.
...
From EncRelSeq Require Import SLInterface2.
