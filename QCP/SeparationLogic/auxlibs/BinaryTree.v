...
Definition map: Type := key -> option value.

Definition map_lookup (x: key) (m: map): value :=
  match m x with
  | Some v => v
  | None => 0
  end.

Definition map_insert (x: key) (v: value) (m: map): map :=
  fun k =>
    if x =? k
    then Some v
    else m k.

Definition Singleton (k: key) (v: value): key -> value -> Prop :=
  fun k0 v0 => k = k0 /\ v = v0.

Fixpoint SetsOf (t: tree): key -> value -> Prop :=
  match t with
  | E => 鈭�
  | T tl k v tr =>
      Singleton k v 鈭� SetsOf tl 鈭� SetsOf tr
  end.

Local Close Scope sets.

Definition rel (t: tree) (m: map): Prop :=
  forall k v,
    SetsOf t k v <-> m k = Some v.

Inductive SearchTree' : key -> tree -> key -> Prop :=
| ST_E : forall lo hi, lo <= hi -> SearchTree' lo E hi
| ST_T: forall lo tl k v tr hi,
    SearchTree' lo tl k ->
    SearchTree' (k + 1) tr hi ->
    SearchTree' lo (T tl k v tr) hi.

Definition SearchTree (t: tree): Prop :=
  exists lo hi, SearchTree' lo t hi.
...
Fixpoint tree_compose (pt: partial_tree) (t: tree) :=
  match pt with
  | nil => t
  | LH k v tr :: pt0 => tree_compose pt0 (T t k v tr)
  | RH k v tl :: pt0 => tree_compose pt0 (T tl k v t)
  end.

Fixpoint tree_lookup (x : key) (t : tree) : value :=
  match t with
  | E => 0
  | T tl k v tr =>
      if x <? k
      then tree_lookup x tl
      else if k <? x
              then tree_lookup x tr
              else v
  end.

Fixpoint tree_insert (x: key) (v: value) (t: tree) : tree :=
  match t with
  | E => T E x v E
  | T a y v' b => if x <? y then T (tree_insert x v a) y v' b
                  else if y <? x then T a y v' (tree_insert x v b)
                  else T a x v b
  end.
...
Fixpoint partial_tree_rep (pt: partial_tree) (p_root p: val): mpred :=
  match pt with
  | nil => !! (p_root = p) && emp
  | LH k v tr :: pt0 =>
      EX p' pr : val,
        !! (Int.min_signed <= k <= Int.max_signed /\
            Int.min_signed <= v <= Int.max_signed) &&
        store_tree_cell p' k v p pr *
        partial_tree_rep pt0 p_root p' *
        tree_rep tr pr
  | RH k v tl :: pt0 =>
      EX p' pl : val,
        !! (Int.min_signed <= k <= Int.max_signed /\
            Int.min_signed <= v <= Int.max_signed) &&
        store_tree_cell p' k v pl p *
        partial_tree_rep pt0 p_root p' *
        tree_rep tl pl
  end.

Fixpoint partial_treebox_rep (pt: partial_tree) (p_root p: val): mpred :=
  match pt with
  | nil => !! (p_root = p) && emp
  | LH k v tr :: pt0 =>
      EX p' q pr : val,
        !! (Int.min_signed <= k <= Int.max_signed /\
            Int.min_signed <= v <= Int.max_signed /\
            p = field_address (Tstruct _tree noattr) [StructField _left] q) &&
        store_tree_ptr p' q *
        store_int (field_address (Tstruct _tree noattr) [StructField _key] q) k *
        store_int (field_address (Tstruct _tree noattr) [StructField _value] q) v *
        store_tree_ptr (field_address (Tstruct _tree noattr) [StructField _right] q) pr *
        partial_treebox_rep pt0 p_root p' *
        tree_rep tr pr
  | RH k v tl :: pt0 =>
      EX p' q pl : val,
        !! (Int.min_signed <= k <= Int.max_signed /\
            Int.min_signed <= v <= Int.max_signed /\
            p = field_address (Tstruct _tree noattr) [StructField _right] q) &&
        store_tree_ptr p' q *
        store_int (field_address (Tstruct _tree noattr) [StructField _key] q) k *
        store_int (field_address (Tstruct _tree noattr) [StructField _value] q) v *
        store_tree_ptr (field_address (Tstruct _tree noattr) [StructField _left] q) pl *
        partial_treebox_rep pt0 p_root p' *
        tree_rep tl pl
  end.

Definition map_rep (m: map) (p: val): mpred :=
  EX t: tree, !! (rel t m /\ SearchTree t) && tree_rep t p.
...
Proof.
  induction t; unfold tree_rep; fold tree_rep; intros.
  - entailer!.
  - Intros pl pr.
    entailer!.
Qed.
...
Proof.
  ...
  destruct t; simpl.
  - entailer!.
  - Intros pl pr. Exists k v t1 t2 pl pr.
    entailer!.
Qed.
...
Lemma store_tree_rep':
  forall k v (pl:val) (pr:val) tl tr p,
    Int.min_signed <= k <= Int.max_signed ->
    Int.min_signed <= v <= Int.max_signed ->
    store_int (field_address (Tstruct _tree noattr) [StructField _key] p) k *
    store_int (field_address (Tstruct _tree noattr) [StructField _value] p) v *
    store_tree_ptr (field_address (Tstruct _tree noattr) [StructField _left] p) pl *
    store_tree_ptr (field_address (Tstruct _tree noattr) [StructField _right] p) pr *
    tree_rep tl pl *
    tree_rep tr pr
    |-- tree_rep (T tl k v tr) p.
Proof.
...
Lemma store_tree_rep1:
  forall k v p,
    Int.min_signed <= k <= Int.max_signed ->
    Int.min_signed <= v <= Int.max_signed ->
    store_tree_cell p k v (Vint (IntRepr 0)) (Vint (IntRepr 0))
    |-- tree_rep (T E k v E) p.
Proof.
  intros.
  unfold tree_rep.
  entailer!.
...
Lemma store_partial_tree_rep_LH: forall pt tr k v p_root p pl pr,
  Int.min_signed <= k <= Int.max_signed ->
  Int.min_signed <= v <= Int.max_signed ->
  store_tree_cell p k v pl pr *
  tree_rep tr pr *
  partial_tree_rep pt p_root p
  |--
    partial_tree_rep (LH k v tr :: pt) p_root pl.
Proof.
  intros.
  unfold partial_tree_rep; fold partial_tree_rep.
  Exists p pr.
  entailer!.
Qed.

Lemma store_partial_tree_rep_RH: forall pt tl k v p_root p pl pr,
  Int.min_signed <= k <= Int.max_signed ->
  Int.min_signed <= v <= Int.max_signed ->
  store_tree_cell p k v pl pr *
  tree_rep tl pl *
  partial_tree_rep pt p_root p
  |--
    partial_tree_rep (RH k v tl :: pt) p_root pr.
Proof.
  intros.
  unfold partial_tree_rep.
  Exists p pl.
  entailer!.
Qed.

Lemma store_partial_treebox_rep_LH: forall pt tr k v b_root b q pr,
  Int.min_signed <= k <= Int.max_signed ->
  Int.min_signed <= v <= Int.max_signed ->
  store_tree_ptr b q *
  store_int (field_address (Tstruct _tree noattr) [StructField _key] q) k *
  ...
  store_tree_ptr (field_address (Tstruct _tree noattr) [StructField _right] q) pr *
  tree_rep tr pr *
  partial_treebox_rep pt b_root b
  |--
  partial_treebox_rep (LH k v tr :: pt) b_root
    (field_address (Tstruct _tree noattr) [StructField _left] q).
Proof.
  intros.
  unfold partial_treebox_rep; fold partial_treebox_rep.
  Exists b q pr.
  entailer!.
Qed.

Lemma store_partial_treebox_rep_RH: forall pt tl k v b_root b q pl,
  Int.min_signed <= k <= Int.max_signed ->
  Int.min_signed <= v <= Int.max_signed ->
  store_tree_ptr b q *
  store_int (field_address (Tstruct _tree noattr) [StructField _key] q) k *
  ...
  partial_treebox_rep pt b_root b
  |--
  partial_treebox_rep (RH k v tl :: pt) b_root
    (field_address (Tstruct _tree noattr) [StructField _right] q).
Proof.
  intros.
  unfold partial_treebox_rep; fold partial_treebox_rep.
  Exists b q pl.
...
Lemma tree_rep_partial_tree_rep: forall t pt p_root p,
  tree_rep t p * partial_tree_rep pt p_root p |--
  tree_rep (tree_compose pt t) p_root.
Proof.
  intros.
  revert p t; induction pt; intros.
  + simpl.
    entailer!.
  + destruct a; simpl.
    - Intros p' pr.
...
Lemma tree_rep_partial_treebox_rep: forall t pt b_root b p,
  store_tree_ptr b p * tree_rep t p * partial_treebox_rep pt b_root b |--
  EX p_root,
    store_tree_ptr b_root p_root * tree_rep (tree_compose pt t) p_root.
Proof.
  intros.
  revert t b p; induction pt; simpl; intros.
  + Exists p.
    entailer!.
  + destruct a.
    - Intros b0 p0 pr.
...
Arguments partial_tree_rep: simpl never.
Arguments map_rep: simpl never.

Lemma SearchTree_left: forall tl k v tr,
  SearchTree (T tl k v tr) ->
  SearchTree tl.
Proof.
  intros.
  destruct H as [lo [hi ?] ].
  inversion H; subst.
  do 2 eexists; exact H6.
Qed.

Lemma SearchTree_right: forall tl k v tr,
  SearchTree (T tl k v tr) ->
  SearchTree tr.
Proof.
  intros.
...
Lemma SearchTree'_order: forall t lo hi,
  SearchTree' lo t hi ->
  lo <= hi.
Proof.
  intros.
...
Lemma SearchTree'_SetsOf: forall lo hi t k v,
  SearchTree' lo t hi ->
  SetsOf t k v ->
  lo <= k < hi.
Proof.
  intros.
  induction H; simpl in H0; sets_unfold in H0.
  + tauto.
  + unfold Singleton in H0.
    destruct H0 as [ [? | ?] | ?].
    - apply SearchTree'_order in H.
      apply SearchTree'_order in H1.
      lia.
    - apply SearchTree'_order in H1.
      apply IHSearchTree'1 in H0.
      lia.
    - apply SearchTree'_order in H.
      apply IHSearchTree'2 in H0.
      lia.
Qed.
               
Lemma SearchTree_SetsOf_left: forall tl k v tr k0 v0,
  SearchTree (T tl k v tr) ->
  SetsOf tl k0 v0 ->
  k0 < k.
Proof.
  ...
  destruct H as [lo [hi ?] ].
  inversion H; subst.
  pose proof SearchTree'_SetsOf _ _ _ _ _ H7 H0.
  lia.
Qed.

Lemma SearchTree_SetsOf_right: forall tl k v tr k0 v0,
  SearchTree (T tl k v tr) ->
  SetsOf tr k0 v0 ->
  k < k0.
Proof.
  ...
  destruct H as [lo [hi ?] ].
  inversion H; subst.
  pose proof SearchTree'_SetsOf _ _ _ _ _ H8 H0.
  lia.
Qed.
               
Definition filter_map (m: map) (f: Z -> bool): map :=
  fun k => if f k then m k else None.

Lemma filter_left_rel: forall tl k v tr m,
  SearchTree (T tl k v tr) ->
  rel (T tl k v tr) m ->
  rel tl (filter_map m (fun k0 => k0 <? k)).
Proof.
  unfold rel.
  unfold filter_map.
  intros.
  specialize (H0 k0 v0).
  simpl in H0; sets_unfold in H0; unfold Singleton in H0.
  ...
  + simpl_if.
    ...
    - lia.
    - tauto.
    - pose proof SearchTree_SetsOf_right _ _ _ _ _ _ H H2.
      lia.
  + simpl_if.
    assert (SetsOf tl k0 v0 -> False). {
      intros.
      pose proof SearchTree_SetsOf_left _ _ _ _ _ _ H H2.
      lia.
    }
    assert (None = Some v0 -> False) by congruence.
    tauto.
Qed.

Lemma filter_right_rel: forall tl k v tr m,
  SearchTree (T tl k v tr) ->
  rel (T tl k v tr) m ->
  rel tr (filter_map m (fun k0 => k <? k0)).
Proof.
  unfold rel.
  unfold filter_map.
  intros.
  specialize (H0 k0 v0).
  simpl in H0; sets_unfold in H0; unfold Singleton in H0.
  destruct (classic (k < k0)).
  + simpl_if.
    rewrite <- H0.
    split; [tauto | intros].
    destruct H2 as [ [? | ?] | ?].
    - lia.
    - pose proof SearchTree_SetsOf_left _ _ _ _ _ _ H H2. lia.
    - tauto.
  + simpl_if.
    assert (SetsOf tr k0 v0 -> False). {
      intros.
      pose proof SearchTree_SetsOf_right _ _ _ _ _ _ H H2.
      lia.
    }
...
Lemma rel_tree_node: forall tl k v tr m,
  rel (T tl k v tr) m ->
  map_lookup k m = v.
Proof.
  intros.
  ...
  assert (m k = Some v) by tauto.
  unfold map_lookup.
  rewrite H0; reflexivity.
Qed.

Lemma tree_lookup_correct: forall t m k0,
  rel t m ->
  SearchTree t ->
  map_lookup k0 m = tree_lookup k0 t.
Proof.
  intros.
  revert m H; induction t; simpl; intros.
  + unfold rel in H.
    simpl in H.
    specialize (H k0).
    unfold map_lookup.
    ...
    - specialize (H v).
      ...
      tauto.
    - reflexivity.
  + destruct (classic (k0 < k)); [| destruct (classic (k < k0))].
    - pose proof filter_left_rel _ _ _ _ _ H0 H.
      rewrite <- (IHt1 ltac:(eapply SearchTree_left, H0) _ H2).
      simpl_if.
      unfold map_lookup, filter_map.
      simpl_if.
      reflexivity.
    - pose proof filter_right_rel _ _ _ _ _ H0 H.
      rewrite <- (IHt2 ltac:(eapply SearchTree_right, H0) _ H3).
      simpl_if.
      unfold map_lookup, filter_map.
      simpl_if.
      reflexivity.
    - simpl_if.
      assert (k = k0) by lia.
      subst k0; clear H0 H2.
      apply rel_tree_node in H; tauto.
Qed.

Lemma tree_insert_correct: forall t m k0 v0,
  rel t m ->
  SearchTree t ->
  rel (tree_insert k0 v0 t) (map_insert k0 v0 m) /\
  SearchTree (tree_insert k0 v0 t).
Proof.
Admitted.
